...
// WriteObjectToString : https://docs.python.org/3/c-api/marshal.html#c.PyMarshal_WriteObjectToString
func (pyObject *PyObject) WriteObjectToString() string {
	obj := togo(C.PyMarshal_WriteObjectToString(toc(pyObject), C.Py_MARSHAL_VERSION))
	defer obj.DecRef()
	return PyBytes_AsString(obj)
}

func (pyObject *PyObject) Marshal() []byte {
	return []byte(pyObject.WriteObjectToString())
}

// WriteObjectToFile : https://docs.python.org/3/c-api/marshal.html#c.PyMarshal_WriteObjectToFile
func (pyObject *PyObject) WriteObjectToFile(filename string) error {
	...
	if err != nil {
		return fmt.Errorf("fail to open '%s': %v", filename, err)
	}
	defer C.fclose(cfile)
	C.PyMarshal_WriteObjectToFile(toc(pyObject), cfile, C.Py_MARSHAL_VERSION)
	return nil
}

// ObjectFromString : https://docs.python.org/3/c-api/marshal.html#c.PyMarshal_ReadObjectFromString
func ObjectFromString(data string) *PyObject {
	cdata := C.CString(data)
	defer C.free(unsafe.Pointer(cdata))
	return togo(C.PyMarshal_ReadObjectFromString(cdata, C.Py_ssize_t(len(data))))
}

func Unmarshal(data []byte) *PyObject {
	return ObjectFromString(string(data))
}
